...
int main( int argc, char ** argv){
    ...
    if (argc > 1) {
        ...
        if(fs::exists(file_path)) {
          ...
          output << "x_data is: " << x_data << std::endl;
          output << "y_data is: " << y_data << std::endl;


        } 
        else {
            std::cerr << "File path is incorrect" << file_path << std::endl;
        } }
    else {
            std::cerr << "Please provide a path to a dataset file\n";
        }
